...
AutoPilotPlugin* PX4FirmwarePlugin::autopilotPlugin(VGVehicle* vehicle)
{
...
QStringList PX4FirmwarePlugin::flightModes(VGVehicle* vehicle)
{
...
bool PX4FirmwarePlugin::isCapable(const VGVehicle *vehicle, FirmwareCapabilities capabilities)
{
...
void PX4FirmwarePlugin::initializeVehicle(VGVehicle* vehicle)
{
...
void PX4FirmwarePlugin::pauseVehicle(VGVehicle* vehicle)
{
...
void PX4FirmwarePlugin::guidedModeRTL(VGVehicle* vehicle)
{
    _setFlightModeAndValidate(vehicle, _rtlFlightMode);
}

void PX4FirmwarePlugin::guidedModeLand(VGVehicle* vehicle)
{
    _setFlightModeAndValidate(vehicle, _landingFlightMode);
}

void PX4FirmwarePlugin::guidedModeOrbit(VGVehicle* vehicle, const QGeoCoordinate& centerCoord, double radius, double velocity, double altitude)
{
...
void PX4FirmwarePlugin::_mavCommandResult(int vehicleId, int component, int command, int result, bool noReponseFromVehicle)
{
    ...
    VGVehicle* vehicle = dynamic_cast<VGVehicle*>(sender());
    if (!vehicle) {
    ...
    if (command == MAV_CMD_NAV_TAKEOFF && result == MAV_RESULT_ACCEPTED) {
        ...
        disconnect(vehicle, &VGVehicle::mavCommandResult, this, &PX4FirmwarePlugin::_mavCommandResult);
        if (!vehicle->armed()) {
...
void PX4FirmwarePlugin::guidedModeTakeoff(VGVehicle* vehicle)
{
    ...
    connect(vehicle, &VGVehicle::mavCommandResult, this, &PX4FirmwarePlugin::_mavCommandResult);
    vehicle->sendMavCommand(vehicle->defaultComponentId(),
                            MAV_CMD_NAV_TAKEOFF,
...
void PX4FirmwarePlugin::guidedModeGotoLocation(VGVehicle* vehicle, const QGeoCoordinate& gotoCoord)
{
    if (qIsNaN(vehicle->altitudeAMSL()->rawValue().toDouble())) {
...
void PX4FirmwarePlugin::guidedModeChangeAltitude(VGVehicle* vehicle, double altitudeChange)
{
...
void PX4FirmwarePlugin::startMission(VGVehicle* vehicle)
{

    if (_setFlightModeAndValidate(vehicle, missionFlightMode())) {
        if (!_armVehicleAndValidate(vehicle)) {
            qgcApp()->showMessage(tr("Unable to start mission: VGVehicle rejected arming."));
            return;
        }
    } else {
        qgcApp()->showMessage(tr("Unable to start mission: VGVehicle not ready."));
    }
}

void PX4FirmwarePlugin::setGuidedMode(VGVehicle* vehicle, bool guidedMode)
{
    if (guidedMode) {
...
bool PX4FirmwarePlugin::isGuidedMode(const VGVehicle* vehicle) const
{
...
bool PX4FirmwarePlugin::adjustIncomingMavlinkMessage(VGVehicle* vehicle, mavlink_message_t* message)
{
...
void PX4FirmwarePlugin::_handleAutopilotVersion(VGVehicle* vehicle, mavlink_message_t* message)
{
    Q_UNUSED(vehicle);
...
bool PX4FirmwarePlugin::vehicleYawsToNextWaypointInMission(const VGVehicle* vehicle) const
{
