...
bool JsonHelper::loadPolygon(const QJsonArray& polygonArray, QList<QGeoCoordinate> &list, QObject *, QString& errorString)
{
    for (int i=0; i<polygonArray.count(); i++) {
        ...
        list.append(pointCoord);
    }

    return true;
}

void JsonHelper::savePolygon(const QList<QGeoCoordinate> &list, QJsonArray& polygonArray)
{
    for (int i=0; i<list.count(); i++)
	{
        QGeoCoordinate vertex = list.at(i);
