...
Rectangle
{
    ...
    function clickBlt(txt) {
        if (!linkManager.isLinked(txt, LinkCommand.TypeBluetooth)) {
            bltManager.shutdown()
            linkManager.shutdown(LinkCommand.TypeBluetooth)
            linkCmd.name = txt
            linkManager.endCreateLinkCmd(linkCmd)
            linkCmd = null
        }
            ...
            Column {
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        ...
                        height:                     txt.height*2
                        color:                      linkManager.isLinked(modelData, LinkCommand.TypeBluetooth)?"#8FDF8F":"#F0FFF0"
                        border.width: 1
                        border.color:               "#003f0f"
                        Text {
                            id:                         txt
