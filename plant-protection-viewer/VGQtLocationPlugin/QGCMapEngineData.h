...
public:
    ...
    virtual TaskType    type            ()const { return _type; }
...
public:
    ...
    QGCCachedTileSet*   tileSet ()const { return _tileSet; }
...
public:
    ...
    QGCCacheTile*   tile()const { return _tile; }
...
signals:
    void            tileListFetched  (const QList<QGCTile*> &tiles);
...
public:
    ...
    QString             hash    ()const { return _hash; }
    qulonglong          setID   ()const { return _setID; }
    QGCTile::TyleState  state   ()const { return _state; }
...
//added by wanekrui 20160806
class QGCExportTileSetTask : public QGCMapTask
{
    Q_OBJECT
public:
    QGCExportTileSetTask(qulonglong setID)
        : QGCMapTask(QGCMapTask::taskExportTileSet)
        , _setID(setID)
    {}

    qulonglong  setID() { return _setID; }

    void setTileSetExported(const QString &setName, const QList<QGCCacheTile*> &tiles)
    {
        emit tileSetExported(QString::number(_setID), setName, tiles);
    }

signals:
    void tileSetExported(const QString &setID, const QString &setName, const QList<QGCCacheTile*> &tiles);

private:
    qulonglong  _setID;
};

class QGCImportTileSetTask : public QGCMapTask
{
    Q_OBJECT
public:
    QGCImportTileSetTask(qulonglong setID)
        : QGCMapTask(QGCMapTask::taskImportTileSet)
        , _setID(setID)
    {}

    qulonglong  setID()const { return _setID; }
    QList<QGCCacheTile*> tiles()const { return _tiles; }
    QString tileSetName()const { return _tileSetName; }

    void setTileSetImported(const QString &setName, const QList<QGCCacheTile*> &tiles)
    {
        _tiles = tiles;
        _tileSetName = setName;
        //emit tileSetImported(QString::number(_setID), setName, tiles);
    }

signals:
    //void tileSetImported(QString setID, QString setName, QList<QGCCacheTile*> tiles);

private:
    qulonglong  _setID;
    QList<QGCCacheTile*> _tiles;
    QString     _tileSetName;
};
