...
//#include "QMLControl/VGMapEngineManager.h"
...
//-----------------------------------------------------------------------------
QString
QGCCachedTileSet::downloadStatus()const
{
    if(_defaultSet) {
...
QGCCachedTileSet::createDownloadTask()
{
    ...
    connect(task, &QGCMapTask::error, this, &QGCCachedTileSet::taskError);
...
//-----------------------------------------------------------------------------
void
QGCCachedTileSet::_tileListFetched(const QList<QGCTile *> &tiles)
{
    _batchRequested = false;
    //-- Done?
...
QGCCachedTileSet::_networkReplyFinished()
{
    ...
    if(!reply) {
        qWarning() << "VGMapEngineManager::networkReplyFinished() NULL Reply";
        return;
    }
    //-- Get tile hash
    ...
    if(!hash.isEmpty()) {
        if(_replies.contains(hash)) {
            ...
            qWarning() << "VGMapEngineManager::networkReplyFinished() Reply not in list: " << hash;
        }
        if (reply->error() != QNetworkReply::NoError) {
            qWarning() << "VGMapEngineManager::networkReplyFinished() Error:" << reply->errorString();
            return;
        }
        ...
        qWarning() << "VGMapEngineManager::networkReplyFinished() Empty Hash";
    }
    reply->deleteLater();
}
...
QGCCachedTileSet::_networkReplyError(QNetworkReply::NetworkError error)
{
    ...
    if(!hash.isEmpty()) {
        if(_replies.contains(hash)) {
            ...
            qWarning() << "VGMapEngineManager::networkReplyError() Reply not in list: " << hash;
        }
        if (error != QNetworkReply::OperationCanceledError) {
            qWarning() << "VGMapEngineManager::networkReplyError() Error:" << reply->errorString();
        }
        QGCUpdateTileDownloadStateTask* task = new QGCUpdateTileDownloadStateTask(_id, QGCTile::StateError, hash);
        getQGCMapEngine()->addTask(task);
    } else {
        qWarning() << "VGMapEngineManager::networkReplyError() Empty Hash";
    }
    //-- Setup a new download
    _prepareDownload();
    reply->deleteLater();
