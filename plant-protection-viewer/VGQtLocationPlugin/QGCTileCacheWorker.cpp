...
QGCCacheWorker::run()
{
    qDebug() << "1--_valid = " << _valid << ",failed = " << _failed;
    if(!_valid && !_failed) {
        _init();
    }
    qDebug() << "2--_valid = " << _valid << ",failed = " << _failed;
    if(_valid) {
        ...
        _valid = _db->open();
        qDebug() << "--run _databasePath " << _databasePath;
    }
    while(true) {
        QGCMapTask* task;
        if(_taskQueue.count()) {
            ...
            switch(task->type())
            {
                ...
                case QGCMapTask::taskDeleteTileSet:
                    _deleteTileSet(task);
                    break;
                case QGCMapTask::taskExportTileSet:
                    _exportTileSet(task);
                    break;
...
//added by wankerui 20160806
void QGCCacheWorker::_exportTileSet(QGCMapTask *mtask)
{
    if(!_valid) {
        mtask->setError("No Cache Database");
        return;
    }
    QList<QGCCacheTile*> tiles;
    QGCExportTileSetTask* task = static_cast<QGCExportTileSetTask*>(mtask);
    QString tileSetName = "";
    QSqlQuery query(*_db);
    QString s;
    s = QString("Select name FROM TileSets WHERE setID = %1").arg(task->setID());
    if(query.exec(s)) {
        while(query.next())
        {
            tileSetName = query.value(0).toString();
        }
    }

    if(tileSetName.isEmpty())
    {
        mtask->setError("NoExist the tileSet");
        return;
    }

    s = QString("Select * FROM Tiles WHERE tileID IN (SELECT tileID FROM SetTiles WHERE setID = %1)").arg(task->setID());
    if(query.exec(s)) {
        while(query.next())
        {
            QGCCacheTile* tile = new QGCCacheTile(
                        query.value("hash").toString(),
                        query.value("tile").toByteArray(),
                        query.value("format").toString(),
                        (UrlFactory::MapType)query.value("type").toInt(),
                        query.value("tileID").toString().toLong()
                        );
            tiles.append(tile);
        }
    }
    task->setTileSetExported(tileSetName, tiles);
}

//导入数据库
void QGCCacheWorker::_importTileSet(QGCMapTask *mtask)
{
    if(!_valid) {
        mtask->setError("No Cache Database");
        return;
    ...
    QGCImportTileSetTask* task = static_cast<QGCImportTileSetTask*>(mtask);

    QList<QGCCacheTile*> tiles = task->tiles();
    QString tileSetName = task->tileSetName();
    ...
    QSqlQuery query(*_db);
    quint64 defaultSetID = _getDefaultTileSet();
    _db->transaction();

    qlonglong tilesCount = tiles.size();
    for(qlonglong i = 0; i < tilesCount; i++)
    {
        QGCCacheTile* tile = tiles.at(i);
        ...
        {
            quint64 tileID = query.lastInsertId().toULongLong();
            quint64 setID = defaultSetID/*tile->set() == UINT64_MAX ? defaultSetID : tile->set()*/;
            QString s = QString("INSERT INTO SetTiles(tileID, setID) VALUES(%1, %2)").arg(tileID).arg(setID);
            query.prepare(s);
            if(!query.exec())
            {
    ...
    emit sigImportMapFinished(defaultSetID);
}
...
{
    _failed = false;
    qDebug() << "3--_valid = " << _valid << ",failed = " << _failed;
    if(!_databasePath.isEmpty()) {
        qDebug() << "1--_databasePath = " << _databasePath;
    ...
    qDebug() << "4--_valid = " << _valid << ",failed = " << _failed;
    _testInternet();
    return _failed;
}
...
{
    qDebug() << "2--_createDB = " << _databasePath;
    QSqlQuery query(*_db);
    if(!query.exec(
    ...
    _failed = !_valid;
    qDebug() << "5--_valid = " << _valid << ",failed = " << _failed;
}
