...
{
    if (networkManager)
        connect(networkManager, &QObject::destroyed, this, &QGeoTiledMapReplyQGC::onNetDestroyed);
...
//-----------------------------------------------------------------------------
void
QGeoTiledMapReplyQGC::cacheError(QGCMapTask::TaskType type, const QString &/*errorString*/)
{
    if (!_networkManager)
        return;

    if(
#ifndef Q_OS_WIN //win10家庭版QNetworkAccessManager没有连接出现networkAccessible为NotAccessible
		_networkManager->networkAccessible() < QNetworkAccessManager::Accessible ||
#endif
		!getQGCMapEngine()->isInternetActive())
	{
		_returnBadTile();
	}
	else
...
#endif
        if (_reply = _networkManager->get(_request))
        {
            _reply->setParent(this);
            connect(_reply, &QObject::destroyed, this, &QGeoTiledMapReplyQGC::onNetDestroyed);
...
void QGeoTiledMapReplyQGC::onNetDestroyed(QObject *obj)
{
    if (_networkManager == obj)
    {
